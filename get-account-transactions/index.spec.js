...
describe('Get Accounting Transaction', () => {
    before(async () => {
    ...
    it('should throw 401 error if the merchantID is not linked to user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/account-transactions`, {
                json: true,
                headers: {
    ...
    it('should throw error if the invalid date range provided', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/account-transactions?fromDate=2221-2331-90&toDate=2018-11-16`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should not return account-transactions of specified merchantID', async () => {
        ...
        const results = await request.get(`${helpers.API_URL}/api/v1/merchants/${sampleMerchantID}/account-transactions`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
