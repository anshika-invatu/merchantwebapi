...
describe('Update Teams', () => {
    ...
    it('should throw error if id is incorrect', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/123/team/123`, {
                json: true,
                headers: {
    ...
    it('should update doc when all validation pass', async () => {
        ...
        const result = await request.patch(`${helpers.API_URL}/api/v1/merchants/${sampleTeams.merchantID}/team/${sampleTeams._id}`, {
            body: {
                name: 'test'
            },
            json: true,
        ...
        expect(result.description).to.be.equal('Successfully updated the document');
    });

    it('should not update doc when team not exist', async () => {

        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/${sampleTeams.merchantID}/team/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                },
