...
describe('Get Bookings', () => {
    ...
    it('should return status code 400 when id not correct', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/bookings/123`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should return doc document when all validation passes', async () => {
        const result = await request.get(`${helpers.API_URL}/api/v1/bookings/${sampleBookings._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result._id).to.eql(sampleBookings._id);
