...
describe('Get Payment Provider Accounts', () => {
    ...
    it('should return document when all validation passes', async () => {
        ...
        const result = await request.get(helpers.API_URL + '/api/v1/payment-provider-accounts', {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
    ...
    it('should return document when all validation passes', async () => {
        ...
        const result = await request.get(helpers.API_URL + '/api/v1/payment-provider-accounts?merchantID=' + merchantID, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
    ...
    it('should return document when all validation passes', async () => {
        ...
        const result = await request.get(helpers.API_URL + '/api/v1/payment-provider-accounts?paymentProviderAccountID=' + samplePaymentProviderAccounts._id, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
