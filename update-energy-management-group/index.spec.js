...
describe('update-energy-group', () => {
    ...
    it('should throw error if id not correct', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/${sampleEnergyManagementGroups.adminRights[0].merchantID}/energy-management-groups/${123}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should update doc when all validation pass', async () => {
        ...
        const result = await request.patch(`${helpers.API_URL}/api/v1/merchants/${sampleEnergyManagementGroups.adminRights[0].merchantID}/energy-management-groups/${sampleEnergyManagementGroups._id}`, {
            body: {
                name: 'test'
            },
            json: true,
        ...
        expect(result.description).to.equal('Successfully updated the specified energyManagementGroups');
    });

    it('should not get doc when data not exist', async () => {

        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/${sampleEnergyManagementGroups.adminRights[0].merchantID}/energy-management-groups/${uuid.v4()}`, {
                body: {
                    name: 'test'
                },
                json: true,
            ...
            const response = {
                code: 404,
                description: 'The energyManagementGroup specified details in the URL doesn\'t exist.',
