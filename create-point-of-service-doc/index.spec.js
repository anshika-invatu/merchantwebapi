...
describe('Create point-of-service', () => {
    before('', async () => {
    ...
    it('should throw error if the document already exists', async () => {
        try {
            await request.post(helpers.API_URL + '/api/v1/point-of-services', {
                body: samplePointOfService,
                json: true,
                headers: {
            ...
            const response = {
                code: 409,
                description: 'You\'ve requested to create a new pointOfService but a pointOfService with the specified _id field already exists.',
                reasonPhrase: 'DuplicatePointOfServiceError'
            };
    ...
    it('should return status code 400 when request body is null', async () => {
        try {
            ...
            const response = {
                code: 400,
                description: 'You\'ve requested to create a new point-of-service but the request body seems to be empty. Kindly pass the point-of-service to be created using request body in application/json format',
    ...
    it('should create the document when all validation passes', async () => {
        await request
            .delete(`${process.env.DEVICE_API_URL}/api/v1/merchants/${samplePointOfService.merchantID}/point-of-services?pointOfServiceID=${samplePointOfService._id}`, {
                body: samplePointOfService,
                json: true,
                headers: {
                    'x-functions-key': process.env.DEVICE_API_KEY
        ...
        const pointOfService = await request
            .post(`${helpers.API_URL}/api/v1/point-of-services`, {
                body: samplePointOfService,
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
        ...
        expect(pointOfService._id).to.equal(samplePointOfService._id);
    });
