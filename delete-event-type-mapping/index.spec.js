...
describe('Delete Event Type Mapping', () => {
    ...
    it('should throw error if id is not uuid', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/event-type-mapping/123`, {
                json: true,
                headers: {
    ...
    it('should delete doc when all validation pass', async () => {
        ...
        const result = await request.delete(`${helpers.API_URL}/api/v1/event-type-mapping/${sampleEventTypesMapping._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result.description).to.equal('Successfully deleted the specified eventTypeMapping');
    });
    
    it('should not delete doc when module not exist', async () => {

        try {
            await request.delete(`${helpers.API_URL}/api/v1/event-type-mapping/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
