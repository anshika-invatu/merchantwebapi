    ...
    return request.get(`${process.env.USER_API_URL}/api/v1/users/${token._id}`, { //Get User
        ...
        .then(result => {
            if (result && result.length) {
                ...
                if (isBusinessunitAccessible) {
                    return request.delete(`${process.env.MERCHANT_API_URL}/api/${process.env.MERCHANT_API_VERSION}/business-units/${req.params.id}`, {
                        json: true,
                        headers: {
                            'x-functions-key': process.env.MERCHANT_API_KEY
                        }
        ...
        .then(result => {
            if (result) {
                ...
                logMessage.result = 'Delete business-units call completed successfully';
