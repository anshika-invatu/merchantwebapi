...
describe('Get apiKey by merchantID', () => {
    before(async () => {
    ...
    it('should throw 401 error if the merchantID is not linked to user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/api-key?merchantID=${uuid.v4()}`, {
                json: true,
                headers: {
    ...
    it('should throw error if id is invalid', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${sampleMerchantID}/api-key?merchantID=123`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should get apiKey doc when all validation pass', async () => {

        const result = await request.get(`${helpers.API_URL}/api/v1/merchants/${sampleMerchantID}/api-key?merchantID=${sampleAPIKey.merchantID}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result[0]._id).to.be.equal(sampleAPIKey._id);
