...
describe('Get Teams', () => {
    ...
    it('should throw error if id is incorrect', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/123/teams`, {
                json: true,
                headers: {
    ...
    it('should get doc when all validation pass', async () => {
        ...
        const result = await request.get(`${helpers.API_URL}/api/v1/merchants/${sampleTeams.merchantID}/teams`, {
            body: {
                name: 'test'
            },
            json: true,
        ...
        expect(result[0].docType).to.be.equal('teams');
    });
