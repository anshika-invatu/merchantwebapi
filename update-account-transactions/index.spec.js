...
describe('Update account transactions', () => {
    ...
    it('should throw error if request id is not  uuid', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/123/account-transactions/${sampleAccounts._id}?accountTransactionID=${sampleAccountTransaction._id}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should update doc when all validation pass', async () => {
        ...
        const result = await request.patch(`${helpers.API_URL}/api/v1/merchants/${merchantID}/account-transactions/${sampleAccounts._id}?accountTransactionID=${sampleAccountTransaction._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            },
        ...
        expect(result.description).to.be.equal('Successfully updated the specified accountTransactions');
    });

    it('should not update doc when module not exist', async () => {

        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/${merchantID}/account-transactions/${uuid.v4()}?accountTransactionID=${sampleAccountTransaction._id}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
