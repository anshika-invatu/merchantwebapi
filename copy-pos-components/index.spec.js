...
describe('Copy POS Components', () => {
    ...
    it('should throw error when _id is not correct', async () => {
        try {
            await request.patch(helpers.API_URL + `/api/v1/copy-pos-components/${123}`, {
                json: true,
                headers: {
    ...
    it('should throw error on incorrect _id field', async () => {
        try {
            await request.patch(helpers.API_URL + `/api/v1/copy-pos-components/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should copy doc when all cases pass', async () => {
        ...
        const result = await request.patch(helpers.API_URL + `/api/v1/copy-pos-components/${sampleComponents._id}`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
        ...
        expect(result.docType).to.equal('components');
    });
