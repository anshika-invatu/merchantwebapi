...
describe('Delete merchant payout bank accounts', () => {
    beforeEach(async () => {
    ...
    it('should throw error if bank account or merchantID is missing from query string', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/bank-accounts?merchantID=${sampleMerchant._id}`, {
                json: true,
                headers: {
    ...
    it('should throw error if merchantID not linked to user', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/bank-accounts?merchantID=${uuid.v4()}&account=${sampleMerchant.payoutBankAccounts[1].account}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should throw error if account not listed in payoutBankAccounts section', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/bank-accounts?merchantID=${sampleMerchant._id}&account=123`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should delete specified bank account', async () => {

        const result = await request.delete(`${helpers.API_URL}/api/v1/bank-accounts?merchantID=${sampleMerchant._id}&account=${sampleMerchant.payoutBankAccounts[1].account}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(merchant.payoutBankAccounts).instanceof(Array).and.have.lengthOf(1);
    });
