...
describe('Delete BookingWeb', () => {
    before(async () => {
        sampleBookingWeb.countryCode = 'se';
        sampleBookingWeb.pageLink = 'testing-link-test-web2';
    ...
    it('should delete doc when all validation pass', async () => {
        ...
        const response = await request.delete(`${helpers.API_URL}/api/v1/merchants/${merchantID}/booking-web/${sampleBookingWeb._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(response.description).to.equal('Successfully deleted the specified bookingWeb');

    });

    it('should throw 401 error if the documentId is not linked to user', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/booking-web/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
