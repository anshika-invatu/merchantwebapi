...
describe('Update Payment Provider Accounts', () => {
    ...
    it('should throw error on incorrect _id field', async () => {
        try {
            await request.patch(helpers.API_URL + '/api/v1/payment-provider-accounts/123', {
                json: true,
                headers: {
    ...
    it('should throw error when payment-provider-account not exist', async () => {
        try {
            await request.patch(helpers.API_URL + `/api/v1/payment-provider-accounts/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should update document when all validation passes', async () => {
        ...
        const result = await request.patch(helpers.API_URL + `/api/v1/payment-provider-accounts/${samplePaymentProviderAccounts._id}`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
            },
            body: { paymentProviderAccountName: 'test' }
        });
        expect(result).not.to.be.null;
        expect(result.description).to.equal('Successfully updated the specified doc');
    });
