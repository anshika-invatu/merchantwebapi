...
describe('view-partner-network-products', () => {
    ...
    before(async () => {
        ...
        samplePartnerNetwork.products.push(product1);
    ...
    it('should throw error on incorrect _id field', async () => {
        try {
            ...
            await request.get(helpers.API_URL + `/api/v1/view-partner-network-products/${123}?merchantID=${merchantID}`, {
                json: true,
                headers: {
    ...
    it('should throw error when query fields are not provided', async () => {
        try {
            product.issuer.merchantID = merchantID;
            await request.get(helpers.API_URL + `/api/v1/view-partner-network-products/${123}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
            ...
            const response = {
                code: 400,
                description: 'Please provide merchantID',
    ...
    it('should throw error when given merchant not linked with this user', async () => {
        try {
            product.issuer.merchantID = merchantID;
            await request.get(helpers.API_URL + `/api/v1/view-partner-network-products/${123}?merchantID=${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should return products when all validation passes', async () => {
        ...
        product.issuer.merchantID = merchantID;
        const result = await request.get(helpers.API_URL + `/api/v1/view-partner-network-products/${samplePartnerNetwork._id}?merchantID=${merchantID}`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
        ...
        expect(result[0].productID).to.eql(product.productID);
