...
module.exports = (context, req) => {
    ...
    if (!req.query.merchantID || !req.query.productID) {
        utils.setContextResError(
            context,
            new errors.FieldValidationError(
                'Please provide merchantID and productID of product',
                400
            )
        );
    ...
    return request.get(`${process.env.USER_API_URL}/api/v1/users/${utils.decodeToken(req.headers.authorization)._id}`, {
        ...
        if (isMerchantLinked) {
            return request.delete(process.env.VOUCHER_API_URL + `/api/${process.env.VOUCHER_API_VERSION}/partner-network-products/${req.params.partnerNetworkID}?${context.req.originalUrl.split('?')[1]}`, {
                json: true,
                headers: {
