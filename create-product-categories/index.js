...
module.exports = async (context, req) => {
    ...
    try {
        ...
        if (!req.body) {
            utils.setContextResError(
                ...
                new errors.EmptyRequestBodyError(
                    'You\'ve requested to create a new productCategories but the request body seems to be empty. Kindly pass the productCategories to be created using request body in application/json format',
                    400
                )
            );
        ...
        const result = await request.post(`${process.env.PRODUCT_API_URL}/api/${process.env.PRODUCT_API_VERSION}/product-categories`, {
            body: req.body,
            json: true,
            headers: {
