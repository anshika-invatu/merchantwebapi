...
describe('Delete POS Components', () => {
    ...
    it('should throw error when _id is not correct', async () => {
        try {
            await request.delete(helpers.API_URL + `/api/v1/pos-components/${123}`, {
                json: true,
                headers: {
    ...
    it('should throw error on incorrect _id field', async () => {
        try {
            await request.delete(helpers.API_URL + `/api/v1/pos-components/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should delete doc when all cases pass', async () => {
        ...
        const result = await request.delete(helpers.API_URL + `/api/v1/pos-components/${sampleComponents._id}`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
        ...
        expect(result.description).to.equal('Successfully deleted the specified components');
    });
