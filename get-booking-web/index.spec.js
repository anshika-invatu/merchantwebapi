...
describe('Get BookingWeb', () => {
    before(async () => {
        sampleBookingWeb.countryCode = 'se';
        sampleBookingWeb.pageLink = 'testing-link-test-web3';
    ...
    it('should get doc when all validation pass', async () => {
        ...
        const response = await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/booking-web/${sampleBookingWeb._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(response._id).to.equal(sampleBookingWeb._id);
    ...
    it('should throw 401 error if the documentId is not linked to user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/booking-web/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
