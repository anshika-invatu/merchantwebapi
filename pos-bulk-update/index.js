...
module.exports = async (context, req) => {
    ...
    if (!req.body) {
        utils.setContextResError(
            ...
            new errors.EmptyRequestBodyError(
                'You\'ve requested to pos bulk update but the request body seems to be empty. Kindly pass request body in application/json format',
                400
            )
        );
        return Promise.resolve();
    }
    if (!req.body.pointOfServices) {
        utils.setContextResError(
            context,
            new errors.FieldValidationError(
                'Please provide pointOfServices in request body.',
                400
            )
        );
        return Promise.resolve();
    ...
    try {
        ...
        const result = await request.patch(`${process.env.DEVICE_API_URL}/api/${process.env.DEVICE_API_VERSION}/pos-bulk-update`, {
            json: true,
            body: req.body,
            headers: {
