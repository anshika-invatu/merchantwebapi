...
describe('Delete Bookings', () => {
    ...
    it('should return status code 400 when id not correct', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/bookings/123-abc`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should delete doc document when all validation passes', async () => {
        const result = await request.delete(`${helpers.API_URL}/api/v1/bookings/${sampleBookings._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result).to.eql({
            code: 200,
            description: 'Successfully deleted the specified bookings'
        });
