...
describe('Delete deny list', () => {
    ...
    it('should throw error on incorrect id field', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/deny-list/123`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should throw 404 error if the data not in db', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/deny-list/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should throw error if the merchantId is not linked', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/deny-list/${sampleDenyLists._id}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
            ...
            const response = {
                code: 401,
                description: 'This merchant does not have permission to delete the denylist',
    ...
    it('should delete the document when all validation passes', async () => {
        const result = await request.delete(`${helpers.API_URL}/api/v1/merchants/${merchantID}/deny-list/${sampleDenyLists._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result.description).to.equal('Successfully deleted the specified denyList');
    });
