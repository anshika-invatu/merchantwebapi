...
describe('Get Contact', () => {
    ...
    it('should throw error on incorrect merchant id field', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/123/contact/123`, {
                json: true,
                headers: {
    ...
    it('should throw error on incorrect contact id field', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/contact/123`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should return the document when all validation passes', async () => {
        const result = await request.get(`${helpers.API_URL}/api/v1/merchants/${sampleContacts.merchantID}/contact/${sampleContacts._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result._id).to.eql(sampleContacts._id);
    });


    it('should throw error if doc not exist in db.', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${sampleContacts.merchantID}/contact/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
