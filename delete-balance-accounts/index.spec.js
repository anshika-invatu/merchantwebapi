...
describe('Delete balance accounts', () => {
    before(async () => {
    ...
    it('should throw error if merchantId not linked to user', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/balance-accounts/${sampleBalanceAccount._id}?merchantID=${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should throw error if merchantID qurey field is empty', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/balance-accounts/${sampleBalanceAccount._id}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should throw error if blance_id is not present in this merchant.', async () => {
        try {
            ...
            const response = {
                code: 401,
                description: 'This balance account id not found in this merchant.',
    ...
    it('should delete balance account if all velidation pass', async () => {
        ...
        expect(result.description).to.eql('Successfully delete balance account');
