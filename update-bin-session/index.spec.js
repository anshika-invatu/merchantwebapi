...
describe('Update Bin Session', () => {
    ...
    it('should update doc when all validation pass', async () => {
        ...
        const response = await request.patch(`${helpers.API_URL}/api/v1/bin-session/${sampleBinSessions._id}`, {
            json: true,
            headers: {
        ...
        expect(response.description).to.eql('Successfully updated the specified bin session');

    });

    it('should throw 401 error if the documentId is not linked to user', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/bin-session/${uuid.v4()}`, {
                json: true,
                headers: {}
            });
        } catch (error) {
