...
describe('Delete Discounts', () => {
    ...
    it('should return status code 400 when id is not in uuid format', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/discounts/123`, {
                json: true,
                headers: {
    ...
    it('should throw error on unauthenticate user', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/discounts/${uuid.v4()}`, {
                json: true,
                body: {},
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
    ...
    it('should create doc document when all validation passes', async () => {
        const result = await request.delete(`${helpers.API_URL}/api/v1/discounts/${sampleDiscounts._id}`, {
            json: true,
        ...
        expect(result.description).to.equal('Successfully deleted the specified discount');
