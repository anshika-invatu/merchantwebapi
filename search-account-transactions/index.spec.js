...
describe('Search account transactions', () => {
    ...
    it('should throw error if request id is not  uuid', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/merchants/123/search-account-transactions/${sampleAccounts._id}?accountTransactionID=${sampleAccountTransaction._id}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should return doc when all validation pass', async () => {
        ...
        const result = await request.post(`${helpers.API_URL}/api/v1/merchants/${merchantID}/search-account-transactions/${sampleAccounts._id}?accountTransactionID=${sampleAccountTransaction._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result[0].docType).to.be.equal('accountTransactions');
    });
