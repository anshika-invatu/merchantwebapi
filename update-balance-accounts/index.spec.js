...
describe('Update balance accounts', () => {
    before(async () => {
    ...
    it('should throw error if merchantId not linked to user', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/balance-accounts/${sampleBalanceAccount._id}?merchantID=${uuid.v4()}`, {
                body: sampleBalanceAccount,
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
    ...
    it('should throw error if merchantID qurey field is empty', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/balance-accounts/${sampleBalanceAccount._id}`, {
                body: sampleBalanceAccount,
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
    ...
    it('should update balance account if all velidation pass', async () => {
        ...
        const result = await request.patch(`${helpers.API_URL}/api/v1/balance-accounts/${sampleBalanceAccount._id}?merchantID=${sampleBalanceAccount.issuerMerchantID}`, {
            body: { balanceAccountType: 'abc', balanceAccountName: 'myaccount' },
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
        ...
        expect(result.description).to.eql('Successfully updated balance account');
