...
describe('Create accounts', () => {
    ...
    it('should throw error if request body is empty', async () => {
        try {
            ...
            const response = {
                code: 400,
                description: 'You\'ve requested to create a new accounts but the request body seems to be empty. Kindly pass the accounts to be created using request body in application/json format',
    ...
    it('should create doc when all validation pass', async () => {
        ...
        const result = await request.post(`${helpers.API_URL}/api/v1/accounts`, {
            body: sampleAccounts,
            json: true,
            headers: {
                'Authorization': authToken
        ...
        expect(result._id).to.be.equal(sampleAccounts._id);
    });
    ...
    it('should not create doc when module already exist', async () => {
        ...
        try {
            ...
            const response = {
                code: 409,
                description: 'You\'ve requested to create a new accounts but a accounts with the specified _id field already exists.',
                reasonPhrase: 'DuplicateAccountError'
            };
    ...
    it('should return error when req body don\'t have merchantID', async () => {
        sampleAccounts._id = uuid.v4();
        sampleAccounts.partitionKey = sampleAccounts._id;
        delete sampleAccounts.merchantID;
        try {
            await request.post(`${helpers.API_URL}/api/v1/accounts`, {
                body: sampleAccounts,
                json: true,
