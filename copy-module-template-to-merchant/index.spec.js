...
describe('Copy ModuleTemplates to merchant', () => {

    before(async () => {

        sampleUser.merchants = [{ merchantID: merchantID, roles: 'admin' }, { merchantID: merchantID, roles: 'admin' }];
    ...
    it('should throw error when _id is not correct', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/copy-module-template`, {
                json: true,
                body: {
                    copyFromMerchantID: merchantID,
                    copyToMerchantID: merchantID,
                    moduleTemplateID: '123',
                    newTemplateName: 'test'
                },
                headers: {
