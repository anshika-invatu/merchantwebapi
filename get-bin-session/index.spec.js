...
describe('Get Bin Session', () => {
    ...
    it('should get doc when all validation pass', async () => {
        ...
        const response = await request.get(`${helpers.API_URL}/api/v1/bin-session/${sampleBinSessions._id}`, {
            json: true,
            headers: {
        ...
        expect(response._id).to.equal(sampleBinSessions._id);

    });

    it('should throw 401 error if the documentId is not linked to user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/bin-session/${uuid.v4()}`, {
                json: true,
                headers: {}
            });
        } catch (error) {
