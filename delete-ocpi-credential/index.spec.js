...
describe('Delete OcpiCredential', () => {
    before(async () => {
    ...
    it('should delete doc when all validation pass', async () => {
        ...
        const response = await request.delete(`${helpers.API_URL}/api/v1/merchants/${sampleMerchant._id}/ocpi-credential/${ocpiCredentialID}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(response.description).to.equal('Successfully deleted the specified ocpiCredentials');

    });

    it('should throw 401 error if the documentId is not linked to user', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/ocpi-credential/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
