...
describe('Delete Contact', () => {
    ...
    it('should throw error on incorrect merchant id field', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/123/contact/123`, {
                json: true,
                headers: {
    ...
    it('should throw error on incorrect contact id field', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/contact/123`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should update the document when all validation passes', async () => {
        const result = await request.delete(`${helpers.API_URL}/api/v1/merchants/${sampleContacts.merchantID}/contact/${sampleContacts._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result.description).to.eql('Successfully deleted the specified contact');
    });
