...
describe('Get Contacts by team', () => {
    before(async () => {
        sampleUser.merchants[0].merchantID = sampleMerchantID;
        sampleContacts.merchantID = sampleMerchantID;
        sampleContacts.teamMemberships.teamID = uuid.v4();
        console.log(sampleContacts.teamMemberships[0].teamID);
    ...
    it('should throw error on incorrect merchant id field', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/teams/123/contacts`, {
                json: true,
                headers: {
            ...
            const response = {
                code: 400,
                description: 'The team id specified in the URL does not match the UUID v4 format.',
    ...
    it('should return the document when all validation passes', async () => {
        const result = await request.get(`${helpers.API_URL}/api/v1/teams/${sampleContacts.teamMemberships[0].teamID}/contacts`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
