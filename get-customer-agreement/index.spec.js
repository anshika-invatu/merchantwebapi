...
describe('Get Customer Agreement by id', () => {
    before(async () => {
    ...
    it('should return error when id is incorrect', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/customer-agreement/123`, {
                json: true,
                headers: {
            ...
            const response = {
                code: 400,
                description: 'The customerAgreement id specified in the URL does not match the UUID v4 format.',
    ...
    it('should throw error on unauthenticate user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/customer-agreement/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                }
    ...
    it('should return doc document when all validation passes', async () => {
        const result = await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/customer-agreement/${sampleCustomerAgreements._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
