...
describe('Get TimeSlot', () => {
    ...
    it('should update doc when all validation pass', async () => {
        ...
        const response = await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/timeslots/${sampleTimeSlot._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(response._id).to.equal(sampleTimeSlot._id);
    ...
    it('should throw 401 error if the documentId is not linked to user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/timeslots/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
