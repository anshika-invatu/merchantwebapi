...
describe('Delete accounts', () => {
    ...
    it('should throw error if request id is not  uuid', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/123/accounts/${sampleAccounts._id}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should delete doc when all validation pass', async () => {
        ...
        const result = await request.delete(`${helpers.API_URL}/api/v1/merchants/${merchantID}/accounts/${sampleAccounts._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result.description).to.be.equal('Successfully deleted the specified account');
    });

    it('should not create doc when module already exist', async () => {

        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${merchantID}/accounts/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
