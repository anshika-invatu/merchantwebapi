...
describe('Delete Event Subscriber', () => {
    ...
    it('should delete doc when all validation pass', async () => {
        ...
        const result = await request.delete(`${helpers.API_URL}/api/v1/event-subscriber/${sampleEventSubscribers._id}`, {
            json: true,
            headers: {
        ...
        expect(result.description).to.be.equal('Successfully deleted the specified eventSubscribers.');
    });
    it('should throw error on incorrect id field', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/event-subscriber/123`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should not delete doc when module not exist', async () => {
        ...
        try {
            await request.delete(`${helpers.API_URL}/api/v1/event-subscriber/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
