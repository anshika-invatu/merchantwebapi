...
describe('Update WorkLog', () => {
    ...
    it('should update doc when all validation pass', async () => {
        ...
        const response = await request.patch(`${helpers.API_URL}/api/v1/merchants/${merchantID}/work-log/${sampleWorkLog._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            },
            body: {
                workLogText: 'testing log text'
            }
        });
        expect(response).not.to.null;
        expect(response.description).to.equal('Successfully updated the specified workLog');
    ...
    it('should throw 401 error if the documentId is not linked to user', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/work-log/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
