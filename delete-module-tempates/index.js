...
module.exports = async (context, req) => {
    try {
        ...
        if (isMerchantLinked) {
            const result = await request.delete(`${process.env.DEVICE_API_URL}/api/${process.env.DEVICE_API_VERSION}/merchants/${req.params.id}/module-templates/${req.params.moduleTemplateID}`, {
                json: true,
                headers: {
            ...
            if (result) {
                context.res = {
                    body: {
                        code: 200,
                        description: 'Successfully deleted module-template'
                    }
                };
            }
        } else {
