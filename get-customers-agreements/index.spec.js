...
describe('Get Customer Agreement by merchant id', () => {
    before(async () => {
    ...
    it('should return error when merchant id is incorrect', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/customers-agreements`, {
                json: true,
                headers: {
    ...
    it('should throw error on unauthenticate user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/customers-agreements`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                }
    ...
    it('should return doc document when all validation passes', async () => {
        const result = await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/customers-agreements`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
