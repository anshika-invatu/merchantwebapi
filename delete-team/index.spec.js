...
describe('Delete Teams', () => {
    ...
    it('should throw error if id is incorrect', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/123/team/123`, {
                json: true,
                headers: {
    ...
    it('should delete doc when all validation pass', async () => {
        ...
        const result = await request.delete(`${helpers.API_URL}/api/v1/merchants/${sampleTeams.merchantID}/team/${sampleTeams._id}`, {
            body: sampleTeams,
            json: true,
            headers: {
                'Authorization': authToken
        ...
        expect(result.description).to.be.equal('Successfully deleted the document');
    });

    it('should not delete doc when team not exist', async () => {

        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${sampleTeams.merchantID}/team/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
