    ...
    return request.get(`${process.env.USER_API_URL}/api/v1/users/${token._id}`, { //Get User
        ...
        .then(result => {
            if (result && result.length) {
                ...
                if (isBusinessunitAccessible) {
                    if (result[0].salesPersons && Array.isArray(result[0].salesPersons) && result[0].salesPersons.length) {
                        salesPersonsArray = result[0].salesPersons;
                        return Promise.resolve(true);
                    } else {
                        utils.setContextResError(
                            context,
                            new errors.SalesPersonsNotFoundError(
                                'Sales persons not found',
                                404
                            )
                        );
                        return Promise.resolve();
        ...
        .then(result => {
            if (result) {
                ...
                logMessage.result = 'Get sales persons call completed successfully';
                utils.logInfo(logMessage);
                context.res = {
                    body: salesPersonsArray // returns sales persons field only
                };
            }
        })
