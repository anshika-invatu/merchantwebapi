...
describe('Create points of service', () => {
    ...
    it('should return status code 400 when request body is null', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/points-of-service/${sampleBusinessUnits._id}`, {
                json: true,
                headers: {
            ...
            const response = {
                code: 400,
                description: 'You\'ve requested to create a points of service but the request body seems to be empty. Kindly specify the points of service field to be created using request body in application/json format',
    ...
    it('should throw error if pointOfService field not exist in request body', async () => {
        try {
            ...
            const response = {
                code: 400,
                description: 'Please provide pointOfService field with pointOfServiceID in request body',
    ...
    it('should throw error if the business-units Id is not exist', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/points-of-service/${uuid.v4()}`, {
                body: {
                    pointOfService: {
    ...
    it('should throw error and do not create point of service if business-units merchant Id not linked to user', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/points-of-service/${sampleBusinessUnits2._id}`, {
                json: true,
                body: {
                    pointOfService: {
                        'pointOfServiceID': uuid.v4(),
    ...
    it('should create point of service in business-unit doc when all validation passes', async () => {
        const result = await request.post(`${helpers.API_URL}/api/v1/points-of-service/${sampleBusinessUnits._id}`, {
            body: {
                pointOfService: {
                    'pointOfServiceID': uuid.v4(),
                    'pointOfServiceName': 'Kassa 1',
        ...
        expect(result.isOnline).to.eql(true);
        ...
        expect(businessUnit[0].pointOfService[0].isOnline).to.equal(true);//point of service field
