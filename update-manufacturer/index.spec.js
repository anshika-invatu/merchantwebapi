...
describe('Update Manufacturers', () => {
    ...
    it('should update doc when all validation pass', async () => {
        ...
        const response = await request.patch(`${helpers.API_URL}/api/v1/manufacturers/${sampleManufacturers._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            },
        ...
        expect(response.description).to.equal('Successfully updated the specified manufacturers');
    ...
    it('should throw 401 error if the merchantID is not linked to user', async () => {
        ...
        try {
            await request.patch(`${helpers.API_URL}/api/v1/manufacturers/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
