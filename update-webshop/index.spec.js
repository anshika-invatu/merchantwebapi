...
describe('Update Webshop', () => {
    ...
    it('should return status code 400 when request body is null', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/webshops/${sampleWebshop._id}`, {
                json: true,
                headers: {
            ...
            const response = {
                code: 400,
                description: 'You\'ve requested to update a webshop but the request body seems to be empty. Kindly specify the webshop properties to be updated using request body in application/json format',
    ...
    it('should throw error on incorrect _id field', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/webshops/123`, {
                json: true,
                body: {},
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
    ...
    it('should throw 404 error if the documentId is invalid', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/webshops/${uuid.v4()}`, {
    ...
    it('should update document when all validation passes', async () => {
        const result = await request.patch(`${helpers.API_URL}/api/v1/webshops/${sampleWebshop._id}`, {
            body: { isRedeemed: true },
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
        ...
        // Get sample document
        const webshop = await request.get(`${process.env.MERCHANT_API_URL}/api/${process.env.MERCHANT_API_VERSION}/webshops/${sampleWebshop._id}`, {
            json: true,
            headers: {
                'x-functions-key': process.env.MERCHANT_API_KEY
            }
        ...
        expect(webshop.isRedeemed).to.equal(true);
        expect(webshop.createdDate).not.to.be.null;
        expect(webshop.updatedDate).not.to.be.null;
        expect(webshop.updatedDate).not.to.equal(sampleWebshop.updatedDate);
    });
