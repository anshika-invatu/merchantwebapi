...
describe('Delete Resources', () => {
    ...
    it('should delete doc when all validation pass', async () => {
        ...
        const response = await request.delete(`${helpers.API_URL}/api/v1/merchants/${merchantID}/resources/${sampleResource._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(response.description).to.equal('Successfully deleted the specified resources');

    });

    it('should throw 401 error if the documentId is not linked to user', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/resources/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
