...
describe('Get AccessToken by id', () => {
    ...
    it('should throw error on incorrect id field', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/access-token/123`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should throw error on data not exist', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/access-token/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should return the document when all validation passes', async () => {
        ...
        const result = await request.get(`${helpers.API_URL}/api/v1/access-token/${sampleAccessToken._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
