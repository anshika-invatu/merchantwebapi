...
describe('Delete product', () => {
    ...
    it('should throw error on incorrect id field', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/products/123-abc`, {
                json: true,
                headers: {
    ...
    it('should throw 404 error if the documentId is invalid', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/products/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should delete the document when all validation passes', async () => {
        const product = await request
            .delete(`${helpers.API_URL}/api/v1/products/${sampleProduct._id}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
        ...
        expect(product).to.eql({ description: 'Successfully deleted the specified product' });
