    ...
    return request.get(`${process.env.USER_API_URL}/api/v1/users/${token._id}`, { //Get User
        ...
        .then(result => {
            if (result) {
                ...
                if (isProductAccessible) {
                    return request.delete(`${process.env.PRODUCT_API_URL}/api/${process.env.PRODUCT_API_VERSION}/products/${req.params.id}`, {
                        json: true,
                        headers: {
                            'x-functions-key': process.env.PRODUCT_API_KEY
                        }
        ...
        .then(result => {
            if (result) {
                ...
                logMessage.result = 'Delete product call completed successfully';
