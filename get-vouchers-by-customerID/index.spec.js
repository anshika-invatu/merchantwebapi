...
describe('Get Vouchers by customer', () => {
    ...
    it('should throw error if user not login', async () => {
        try {
            const url = `${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/vouchers/${uuid.v4()}`;
    ...
    it('should throw error if id not in uuid format', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/123/vouchers/123`, {
                json: true,
                headers: {
    ...
    it('should throw error if the documentId is invalid', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/vouchers/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should throw error if the documentId is invalid', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/vouchers/${sampleCustomers._id}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
