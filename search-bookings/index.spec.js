...
describe('Search Bookings', () => {
    ...
    it('should return status code 400 when id not correct', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/merchants/123/search-bookings`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should return doc document when all validation passes', async () => {
        const result = await request.post(`${helpers.API_URL}/api/v1/merchants/${sampleBookings.merchantID}/search-bookings`, {
            json: true,
            headers: {
                'Authorization': authToken
            },
