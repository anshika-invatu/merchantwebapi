...
sampleAccountTransaction.createdDate = new Date();

describe('Get sales by custmer account', () => {
    ...
    it('should throw error if request id is not  uuid', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/merchants/123/sales-by-customer-account`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should return doc when all validation pass', async () => {
        ...
        const result = await request.post(`${helpers.API_URL}/api/v1/merchants/${merchantID}/sales-by-customer-account`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result[0].customerAccountID).to.equal(sampleAccounts._id);
    });
