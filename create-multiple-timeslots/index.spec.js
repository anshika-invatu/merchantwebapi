...
describe('Create TimeSlots', () => {
    ...
    it('should throw error if request body is empty', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/multiple-timeslots`, {
                json: true,
                headers: {
            ...
            const response = {
                code: 400,
                description: 'You\'ve requested to create a new timeslots but the request body seems to be empty or not in array. Kindly pass the timeslots to be created using request body in application/json format',
    ...
    it('should create doc when all validation pass', async () => {
        ...
        const result = await request.post(`${helpers.API_URL}/api/v1/multiple-timeslots`, {
            body: [sampleTimeSlot],
            json: true,
            headers: {
                'Authorization': authToken
        ...
        expect(result[0]._id).to.be.equal(sampleTimeSlot._id);
    });
