...
describe('Get AccessToken', () => {
    ...
    it('should return doc when all validation pass', async () => {
        ...
        const result = await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/access-token`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
    ...
    it('should return the document when all validation passes with all params', async () => {
        ...
        const result = await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/access-token?merchantID=${merchantID}&agreementID=${sampleAccessToken.customerAgreementID}&accountID=${sampleAccessToken.customerAccountID}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
