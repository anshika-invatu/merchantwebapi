...
describe('delete-energy-group', () => {
    ...
    it('should throw error if request body is empty', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${sampleEnergyManagementGroups.adminRights[0].merchantID}/energy-management-groups/${123}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should delete doc when all validation pass', async () => {
        ...
        const result = await request.delete(`${helpers.API_URL}/api/v1/merchants/${sampleEnergyManagementGroups.adminRights[0].merchantID}/energy-management-groups/${sampleEnergyManagementGroups._id}`, {
            body: sampleEnergyManagementGroups,
            json: true,
            headers: {
                'Authorization': authToken
        ...
        expect(result.description).to.equal('Successfully deleted the specified energyManagementGroups');
    });

    it('should not delete doc when data not exist', async () => {

        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${sampleEnergyManagementGroups.adminRights[0].merchantID}/energy-management-groups/${uuid.v4()}`, {
                body: sampleEnergyManagementGroups,
                json: true,
                headers: {
                    'Authorization': authToken
