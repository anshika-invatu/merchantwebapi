...
describe('Create dynamic quickshop', () => {
    before(async () => {
    ...
    it('should throw error if the request body not exists', async () => {
        try {
            await request.post(helpers.API_URL + `/api/v1/merchants/${uuid.v4()}/dynamic-quickshop`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
            ...
            const response = {
                code: 400,
                description: 'The quickShopTemplateID field specified in the request body does not match the UUID v4 format.',
    ...
    it('should create document when all validation passes', async () => {
        ...
        const result = await request.post(helpers.API_URL + `/api/v1/merchants/${sampleMerchant._id}/dynamic-quickshop`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
            ...
            body: {
                quickShopTemplateID: sampleQuickShop._id,
                quickShopName: 'test'
            }
        });
        expect(result).not.to.be.null;
        expect(result.quickShopType).to.eql('dynamic');
    });

    it('should throw error if the document not exists', async () => {
        try {
            await request.post(helpers.API_URL + `/api/v1/merchants/${sampleQuickShop.adminRights[0].merchantID}/dynamic-quickshop`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
            ...
            const response = {
                code: 404,
                description: 'The quickshop template id specified in the URL doesn\'t exist.',
