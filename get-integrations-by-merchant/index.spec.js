...
describe('Get Integrations by merchantID', () => {
    before(async () => {
        ...
        sampleIntegrations.merchantID = merchantID;
    ...
    it('should get doc list when all validation pass', async () => {
        ...
        const response = await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/integrations`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(response[0]._id).to.equal(sampleIntegrations._id);
        
    });

    it('should throw 401 error if the merchantID is not linked to user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/integrations`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
