...
describe('Delete SmartChargingProfiles', () => {
    before(async () => {
    ...
    it('should delete doc when all validation pass', async () => {
        const response = await request.delete(`${helpers.API_URL}/api/v1/merchants/${merchantID}/smart-charging-profiles/${sampleSmartChargingProfiles._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(response.description).to.equal('Successfully deleted the specified smartChargingProfiles');

    });

    it('should throw 401 error if the merchantID is not linked to user', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/smart-charging-profiles/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
