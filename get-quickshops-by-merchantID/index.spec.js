...
describe('Get quickshops by merchantID', () => {
    before(async () => {
    ...
    it('should throw error if the request body not exists', async () => {
        try {
            await request.get(helpers.API_URL + '/api/v1/merchants/123/quickshops', {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should create document when all validation passes', async () => {
        ...
        const result = await request.get(helpers.API_URL + `/api/v1/merchants/${sampleMerchant._id}/quickshops`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
        ...
        expect(result[0]._id).to.eql(sampleQuickShop._id);
    });
