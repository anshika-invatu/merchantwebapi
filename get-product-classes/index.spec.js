...
describe('Get product classes', () => {
    ...
    it('should throw 401 error if the merchantID is not linked to user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/product-classes`, {
                json: true,
                headers: {
    ...
    it('should return account list doc of specified merchantID', async () => {
        ...
        const productClasses = await request.get(`${helpers.API_URL}/api/v1/merchants/${sampleMerchantID}/product-classes`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        });
        expect(productClasses).not.to.be.null;
        expect(productClasses).to.be.instanceOf(Array).and.not.have.lengthOf(0);
    });
