...
describe('Update Customer Agreement by id', () => {
    before(async () => {
    ...
    it('should return status code 400 when request body is null', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/${merchantID}/customer-agreement/123`, {
                json: true,
                headers: {
    ...
    it('should throw error on unauthenticate user', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/${merchantID}/customer-agreement/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                }
    ...
    it('should return doc document when all validation passes', async () => {
        const result = await request.patch(`${helpers.API_URL}/api/v1/merchants/${merchantID}/customer-agreement/${sampleCustomerAgreements._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            },
        ...
        expect(result.description).to.eql('Successfully updated the specified customer agreement');
