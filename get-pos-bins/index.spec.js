...
describe('Get POS Bins', () => {
    ...
    it('should throw error on incorrect _id field', async () => {
        try {
            await request.get(helpers.API_URL + '/api/v1/pos-bins/123', {
                json: true,
                headers: {
    ...
    it('should return error when document not exist', async () => {
        ...
        try {
            await request.get(helpers.API_URL + `/api/v1/pos-bins/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should return error when document not exist', async () => {
        ...
        try {
            await request.get(helpers.API_URL + `/api/v1/pos-bins/${samplePointOfService._id}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
