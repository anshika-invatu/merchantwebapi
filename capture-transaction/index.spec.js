...
describe('Capture account transactions', () => {
    ...
    it('should throw error if request id is not  uuid', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/capture-transaction`, {
                json: true,
                headers: {
                    'Authorization': authToken
                },
    ...
    it('should update doc when all validation pass', async () => {
        ...
        const result = await request.post(`${helpers.API_URL}/api/v1/capture-transaction`, {
            json: true,
            headers: {
                'Authorization': authToken
            },
            body: {
                accountTransactionID: sampleAccountTransaction._id,
                amount: 10,
                vatPercent: 12
            }
        });
        expect(result).not.to.be.null;
        expect(result.description).to.be.equal('Successfully updated the specified account transaction');
    });
