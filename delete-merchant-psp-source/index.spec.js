...
describe('Delete merchant psp source', () => {
    before(async () => {
        const merchantArray = [];
        merchantArray.push({
    ...
    it('should throw error on unauthenticate request', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/delete-merchant-psp-source`, {
                json: true,
                headers: {
    ...
    it('should throw error if request body is empty', async () => {
        try {
            ...
            const response = {
                code: 400,
                description: 'Please provide body parameters merchantID and pspSource',
