...
describe('Delete apiKey', () => {
    ...
    it('should throw 401 error if the merchantID is not linked to user', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/api-key/${uuid.v4()}`, {
                json: true,
                headers: {
    ...
    it('should throw error if id is invalid', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${sampleMerchantID}/api-key/123`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should delete apiKey doc when all validation pass', async () => {
        ...
        const result = await request.delete(`${helpers.API_URL}/api/v1/merchants/${sampleMerchantID}/api-key/${sampleAPIKey._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result.description).to.be.equal('Successfully deleted the specified apiKey');
    });
