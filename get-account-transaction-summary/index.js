    ...
    if (!req.query.fromDate || !req.query.toDate) {
        utils.setContextResError(
            ...
            new errors.FieldValidationError(
                'Please provide valid daterange fromDate and toDate in format YYYY-MM-DD.',
                400
            )
        );
            ...
            if (user.merchants[i].merchantID === req.params.id) {   //Validate whether user is allowed to see merchant data or not?
                ...
                return request.get(`${process.env.LEDGERS_API_URL}/api/v1/merchants/${req.params.id}/account-transactions?${queryString}`, {
                    ...
                    if (accountTransaction.length) {
                        accountTransaction = utils.summarizedTransaction(accountTransaction);
                    }
                    const logMessage = {};
                    logMessage.responseTime = `${(new Date() - executionStart)} ms`; // duration in ms
                    logMessage.operation = 'Get';
                    logMessage.result = 'Get account transaction summary call completed successfully';
