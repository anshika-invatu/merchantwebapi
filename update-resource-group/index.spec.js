...
describe('Update Resource Group', () => {
    ...
    it('should update doc when all validation pass', async () => {
        ...
        const response = await request.patch(`${helpers.API_URL}/api/v1/merchants/${merchantID}/resource-group/${sampleResourceGroup._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            },
        ...
        expect(response.description).to.equal('Successfully updated the specified resourceGroup');
    ...
    it('should throw 401 error if the documentId is not linked to user', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/resource-group/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
