...
describe('Delete quickshop', () => {
    ...
    it('should throw error if the request body not exists', async () => {
        try {
            await request.delete(helpers.API_URL + `/api/v1/merchants/${uuid.v4()}/quickshop/123`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should create document when all validation passes', async () => {
        ...
        const result = await request.delete(helpers.API_URL + `/api/v1/merchants/${sampleMerchant._id}/quickshop/${sampleQuickShop._id}`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
        ...
        expect(result).to.eql({ description: 'Successfully deleted the specified quickshop' });
    });

    it('should throw error if the document not exists', async () => {
        try {
            await request.delete(helpers.API_URL + `/api/v1/merchants/${sampleQuickShop.adminRights[0].merchantID}/quickshop/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
