...
describe('Get Module-templates', () => {
    ...
    it('should throw error on unauthenticate user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/module-templates`, {
                json: true,
                headers: {
    ...
    it('should return modules when all cases are pass', async () => {
        ...
        const result = await request.get(`${helpers.API_URL}/api/v1/module-templates`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
        ...
        expect(result[0].idlePage).to.be.undefined;
        expect(result[0].menuPage).to.be.undefined;
        expect(result[0].externalDisplay).to.be.undefined;
        expect(result[0].modulesFlow).to.be.undefined;
        expect(result[0].actionTriggers).to.be.undefined;
        expect(result[0].dataSources).to.be.undefined;
        expect(result[0].properties).to.be.undefined;
    });
