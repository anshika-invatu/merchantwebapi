...
describe('Refund account transactions', () => {
    ...
    it('should throw error if request id is not  uuid', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/refund-transaction/123`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should refund doc when all validation pass', async () => {

        const result = await request.post(`${helpers.API_URL}/api/v1/refund-transaction/${sampleAccountTransaction._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result.description).to.be.equal('Successfully refunded the specified account transaction');
    });

    it('should not update doc when module not exist', async () => {

        try {
            await request.post(`${helpers.API_URL}/api/v1/refund-transaction/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
            ...
            const response = {
                code: 404,
                description: 'The account transaction specified details in the URL doesn\'t exist.',
                reasonPhrase: 'AccountTransactionNotFoundError'
            };
