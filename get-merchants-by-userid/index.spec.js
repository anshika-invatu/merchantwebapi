...
describe('Get Merchants By UserID', () => {
    before(async () => {
    ...
    it('should throw error on incorrect id field', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/users/123-abc/merchants`, {
                json: true,
                headers: {
    ...
    it('should throw 401 error if the user id provided is invalid', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/users/${uuid.v4()}/merchants`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should return the merchants linked to the userid', async () => {
        const merchants = await request
            .get(`${helpers.API_URL}/api/v1/users/${sampleUser._id}/merchants`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
        ...
        expect(merchants).to.be.instanceOf(Array).and.have.lengthOf(3);
    });
