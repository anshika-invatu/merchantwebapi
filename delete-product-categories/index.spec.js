...
describe('Delete Product Categories', () => {
    ...
    it('should delete doc when all validation pass', async () => {
        ...
        const result = await request.delete(`${helpers.API_URL}/api/v1/merchants/${merchantID}/product-categories/${sampleProductCategories._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result.description).to.be.equal('Successfully deleted the specified productCategories');
    });

    it('should not delete doc when db don\'t have doc with this id.', async () => {

        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${merchantID}/product-categories/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should return error when id is not in uuid format', async () => {
        ...
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${merchantID}/product-categories/123`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
