...
describe('Copy quickshop', () => {
    ...
    it('should throw error if the request body not exists', async () => {
        try {
            await request.patch(helpers.API_URL + `/api/v1/merchants/${uuid.v4()}/copy-quickshop/123`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should create document when all validation passes', async () => {
        ...
        const result = await request.patch(helpers.API_URL + `/api/v1/merchants/${sampleMerchant._id}/copy-quickshop/${sampleQuickShop._id}`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
        ...
        expect(result.docType).to.eql('quickShop');
        expect(result.quickShopName).to.eql(`${sampleQuickShop.quickShopName}-copy`);

    });

    it('should throw error if the document not exists', async () => {
        try {
            await request.patch(helpers.API_URL + `/api/v1/merchants/${sampleQuickShop.adminRights[0].merchantID}/copy-quickshop/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
