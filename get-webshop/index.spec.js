...
describe('Get Webshop', () => {
    ...
    it('should throw error on incorrect id field', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/webshops/123-abc`, {
                json: true,
                headers: {
    ...
    it('should throw 404 error if the documentId is invalid', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/webshops/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should return the document when all validation passes', async () => {
        const webshop = await request
            .get(`${helpers.API_URL}/api/v1/webshops/${sampleWebshop._id}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
        ...
        expect(webshop._id).to.equal(sampleWebshop._id);
    });
