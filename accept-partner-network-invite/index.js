...
module.exports = (context, req) => {
    ...
    if (!req.body.partnerNetworkID || !req.body.merchantID) {
        utils.setContextResError(
            context,
            new errors.FieldValidationError(
                'Please provide req body parameters partnerNetworkID and merchantID',
                400
            )
        );
        return Promise.reject();
    ...
    return request.get(`${process.env.USER_API_URL}/api/v1/users/${utils.decodeToken(req.headers.authorization)._id}`, {
        ...
        if (isMerchantLinked) {
            return request.patch(process.env.VOUCHER_API_URL + `/api/${process.env.VOUCHER_API_VERSION}/accept-partner-network-invite`, {
                json: true,
                headers: {
