...
describe('Update account transactions', () => {
    ...
    it('should throw error if request id is not  uuid', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/123/account-transactions/${sampleAccounts._id}?accountTransactionID=${sampleAccountTransaction._id}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should return doc when all validation pass', async () => {
        ...
        const result = await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/account-transactions/${sampleAccounts._id}?accountTransactionID=${sampleAccountTransaction._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
    ...
    it('should not create doc when module already exist', async () => {
        ...
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/account-transactions/${uuid.v4()}?accountTransactionID=${sampleAccountTransaction._id}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
