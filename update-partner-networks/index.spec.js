...
describe('Update Partner Network', () => {
    ...
    it('should return status code 400 when request body is null', async () => {
        try {
            await request.patch(helpers.API_URL + `/api/v1/merchants/${uuid.v4()}/partner-networks/` + samplePartnerNetwork._id, {
                json: true,
                headers: {
            ...
            const response = {
                code: 400,
                description: 'You\'ve requested to update partnerNetwork but the request body seems to be empty. Kindly pass the partnerNetwork fields to be updated using request body in application/json format',
    ...
    it('should throw error on incorrect _id field', async () => {
        try {
            await request.patch(helpers.API_URL + `/api/v1/merchants/${uuid.v4()}/partner-networks/` + samplePartnerNetwork._id, {
                ...
                body: {
                    _id: 123,
                    ownerMerchantID: samplePartnerNetwork.ownerMerchantID
                },
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should update document when all validation passes', async () => {
        ...
        const result = await request.patch(helpers.API_URL + `/api/v1/merchants/${merchantID}/partner-networks/` + samplePartnerNetwork._id, {
            json: true,
            body: samplePartnerNetwork,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
