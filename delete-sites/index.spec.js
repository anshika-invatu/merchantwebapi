...
describe('Delete Sites', () => {
    ...
    it('should delete doc when all validation pass', async () => {
        ...
        const result = await request.delete(`${helpers.API_URL}/api/v1/sites/${sampleSites._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result.description).to.be.equal('Successfully deleted the specified site');
    });

    it('should not delete site when db don\'t have site with this id.', async () => {

        try {
            await request.delete(`${helpers.API_URL}/api/v1/sites/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should return error when id is not in uuid format', async () => {
        ...
        try {
            await request.delete(`${helpers.API_URL}/api/v1/sites/123`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
