...
describe('Delete DynamicPricingGroup', () => {
    before(async () => {
    ...
    it('should delete doc when all validation pass', async () => {
        ...
        const response = await request.delete(`${helpers.API_URL}/api/v1/merchants/${merchantID}/dynamic-pricing-group/${sampleDynamicPricingGroups._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(response.description).to.equal('Successfully deleted the specified dynamicPricingGroups');

    });

    it('should throw 401 error if the documentId is not linked to user', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/dynamic-pricing-group/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
