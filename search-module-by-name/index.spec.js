...
sampleModules.issuedByMerchantID = merchantID;
let authToken = '';
...
sampleModules.moduleDescription = {
    'sv-SE': {
        'text': 'Testing123'
    },
    'en-US': {
...
describe('Search Module Template', () => {
    ...
    it('should return doc when all cases pass with languageCode sv-SE', async () => {
        ...
        const pos = await request.get(`${helpers.API_URL}/api/v1/search-modules?name=Testing123`, {
            json: true,
            headers: {
        ...
        expect(pos[0].docType).to.equal('modules');
        expect(pos[0]._id).to.equal(sampleModules._id);
    });

    it('should return doc when all cases pass with all query params', async () => {
       
        const pos = await request.get(`${helpers.API_URL}/api/v1/search-modules?description=Testing123`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
