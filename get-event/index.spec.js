...
describe('Get Event', () => {
    ...
    it('should throw error if id is not uuid.', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/event/123-abc`, {
                json: true,
                headers: {
    ...
    it('should get doc when all validation pass', async () => {
        ...
        const result = await request.get(`${helpers.API_URL}/api/v1/event/${sampleEvents._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result._id).to.equal(sampleEvents._id);
    });
    
    it('should not get doc when module not exist', async () => {

        try {
            await request.get(`${helpers.API_URL}/api/v1/event/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
