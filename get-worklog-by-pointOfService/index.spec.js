...
describe('Get WorkLog by pointOfService', () => {
    before(async () => {
    ...
    it('should get doc when all validation pass', async () => {
        ...
        const response = await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/work-log-by-pointOfService/${sampleWorkLog.pointOfServiceID}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(response[0].docType).to.equal('workLog');
    ...
    it('should throw 401 error if the merchantID is not linked to user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/work-log-by-pointOfService/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
