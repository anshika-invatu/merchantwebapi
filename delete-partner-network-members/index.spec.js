...
describe('Delete Partner Network Members', () => {
    ...
    it('should throw error on incorrect _id field', async () => {
        try {
            await request.delete(helpers.API_URL + `/api/v1/partner-network-members/${merchantID1}/partner-networks/${123}?requestedMerchantID=${merchantID}`, {
                json: true,
                headers: {
    ...
    it('should update document when all validation passes', async () => {
        ...
        const result = await request.delete(helpers.API_URL + `/api/v1/partner-network-members/${merchantID1}/partner-networks/${samplePartnerNetwork._id}?requestedMerchantID=${merchantID}`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
