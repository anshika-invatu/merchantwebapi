...
describe('Get Product', () => {
    ...
    it('should throw error on incorrect id field', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${sampleProduct.issuer.merchantID}/product-log/123-abc`, {
                json: true,
                headers: {
    ...
    it('should throw 404 error if the documentId is invalid', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${sampleProduct.issuer.merchantID}/product-log/${uuid.v4()}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
    ...
    it('should return the document when all validation passes', async () => {
        ...
        const productLog = await request.get(`${helpers.API_URL}/api/v1/merchants/${sampleProduct.issuer.merchantID}/product-log/${sampleProduct._id}`, {
            json: true,
            headers: {
                'x-functions-key': process.env.X_FUNCTIONS_KEY,
                'Authorization': authToken
        ...
        expect(productLog).not.to.be.null;
        expect(productLog[0].docType).to.equal('productLog');
    });
