...
describe('get-report-subscribers', () => {
    ...
    it('should return error when merchant id not matched.', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/report-subscribers`, {
                json: true,
                headers: {
    ...
    it('should get doc when all validation pass', async () => {
        ...
        const result = await request.get(`${helpers.API_URL}/api/v1/merchants/${sampleReportSubscribers.merchantID}/report-subscribers`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result[0].docType).to.equal('reportSubscribers');
    });
