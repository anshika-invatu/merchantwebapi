    ...
    return request.get(`${process.env.USER_API_URL}/api/v1/users/${token._id}`, { //Get User
        ...
        .then(result => {
            if (result) {
                ...
                if (isWebshopAccessible) {
                    return request.delete(`${process.env.MERCHANT_API_URL}/api/${process.env.MERCHANT_API_VERSION}/webshops/${req.params.id}`, {
                        json: true,
                        headers: {
                            'x-functions-key': process.env.MERCHANT_API_KEY
                        }
        ...
        .then(result => {
            if (result) {
                ...
                logMessage.result = 'Delete webshop call completed successfully';
