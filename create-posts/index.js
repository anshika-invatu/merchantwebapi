...
module.exports = async (context, req) => {
    ...
    try {
        ...
        if (!req.body) {
            utils.setContextResError(
                ...
                new errors.EmptyRequestBodyError(
                    'You\'ve requested to create a new posts but the request body seems to be empty. Kindly pass the posts to be created using request body in application/json format',
                    400
                )
            );
        ...
        const result = await request.post(`${process.env.CONTENTS_API_URL}/api/${process.env.CONTENTS_API_VERSION}/posts`, {
            body: req.body,
            json: true,
            headers: {
