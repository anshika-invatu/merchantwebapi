...
module.exports = async (context, req) => {
    ...
    try {
        ...
        if (!req.body) {
            utils.setContextResError(
                ...
                new errors.EmptyRequestBodyError(
                    'You\'ve requested to create a new posProperties but the request body seems to be empty. Kindly pass the posProperties to be created using request body in application/json format',
                    400
                )
            );
        ...
        const result = await request.post(`${process.env.DEVICE_API_URL}/api/${process.env.DEVICE_API_VERSION}/pos-properties`, {
            body: req.body,
            json: true,
            headers: {
