...
module.exports = async (context, req) => {
    ...
    try {
        ...
        if (!req.body) {
            utils.setContextResError(
                ...
                new errors.EmptyRequestBodyError(
                    'You\'ve requested to create a new dynamicPricingGroups but the request body seems to be empty. Kindly pass the dynamicPricingGroups to be created using request body in application/json format',
                    400
                )
            );
        ...
        const result = await request.post(`${process.env.ENERGY_DATA_API_URL}/api/${process.env.ENERGY_DATA_API_VERSION}/dynamic-pricing-group`, {
            body: req.body,
            json: true,
            headers: {
