...
describe('Search Resources', () => {
    ...
    it('should update doc when all validation pass', async () => {
        ...
        const response = await request.post(`${helpers.API_URL}/api/v1/merchants/${merchantID}/search-resources`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(response[0]._id).to.equal(sampleResource._id);
    ...
    it('should throw 401 error if the documentId is not linked to user', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/search-resources`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
