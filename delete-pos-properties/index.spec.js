...
describe('Delete Pos Properties', () => {
    before(async () => {
    ...
    it('should delete doc when all validation pass', async () => {
        ...
        const response = await request.delete(`${helpers.API_URL}/api/v1/merchants/${merchantID}/pos-properties/${samplePosProperties._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(response.description).to.equal('Successfully deleted the specified posProperties');

    });

    it('should throw 401 error if the documentId is not linked to user', async () => {
        try {
            await request.delete(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/pos-properties/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
