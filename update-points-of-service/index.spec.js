...
describe('Update points of service', () => {
    before(async () => {
    ...
    it('should return status code 400 when request body is null', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/points-of-service?businessunitID=${sampleBusinessUnits._id}&pointOfServiceID=${pointOfServiceID}`, {
                json: true,
                headers: {
            ...
            const response = {
                code: 400,
                description: 'You\'ve requested to update a points of service but the request body seems to be empty. Kindly specify the points of service field to be updated using request body in application/json format',
    ...
    it('should throw error if pointOfService field not exist in request body', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/points-of-service?businessunitID=${sampleBusinessUnits._id}&pointOfServiceID=${pointOfServiceID}`, {
                json: true,
            ...
            const response = {
                code: 400,
                description: 'Please provide pointOfService field in request body',
    ...
    it('should throw error if the business-units Id is not exist', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/points-of-service?businessunitID=${uuid.v4()}&pointOfServiceID=${pointOfServiceID}`, {
                body: {
                    pointOfService: {
                        'pointOfServiceID': uuid.v4(),
                        'pointOfServiceName': 'Kassa 1',
    ...
    it('should throw error and do not update point of service if business-units merchant Id not linked to user', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/points-of-service?businessunitID=${sampleBusinessUnits2._id}&pointOfServiceID=${pointOfServiceID}`, {
                json: true,
                body: {
                    pointOfService: {
                        'pointOfServiceID': uuid.v4(),
    ...
    it('should update point of service in business-unit doc when all validation passes', async () => {
        const result = await request.patch(`${helpers.API_URL}/api/v1/points-of-service?businessunitID=${sampleBusinessUnits._id}&pointOfServiceID=${pointOfServiceID}`, {
            body: {
                pointOfService: {
                    'pointOfServiceDescription': 'The main POS updated',
                    'pointOfServiceDeviceID': 'FTHY-F345-FF689',
                    'location': 'Floor 2',
                    'latitude': -78.78,
                    'longitude': 20.412,
                    'isOnline': false,
        ...
        expect(businessUnit[0].pointOfService).to.be.instanceOf(Array).and.have.lengthOf(1);
        expect(businessUnit[0].pointOfService[0].isOnline).to.equal(false);//updated point of service field
