...
describe('Get Customer', () => {
    ...
    it('should return status code 400 when customer id not correct', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/customers/123-abc`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should throw error on unauthenticate user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/customers/${uuid.v4()}`, {
                json: true,
                headers: {
    ...
    it('should return doc document when all validation passes', async () => {
        const result = await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/customers/${sampleCustomers._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result._id).to.eql(sampleCustomers._id);
