...
describe('Delete Integration', () => {
    ...
    it('should delete doc when all validation pass', async () => {
        ...
        const result = await request.delete(`${helpers.API_URL}/api/v1/integration/${sampleIntegrations._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result.description).to.be.equal('Successfully deleted the specified integrations');
    });

    it('should not delete integration when db don\'t have integration with this id.', async () => {

        try {
            await request.delete(`${helpers.API_URL}/api/v1/integration/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should return error when id is not in uuid format', async () => {
        ...
        try {
            await request.delete(`${helpers.API_URL}/api/v1/integration/123`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
