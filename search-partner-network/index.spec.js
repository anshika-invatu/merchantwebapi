...
describe('Search Partner Network', () => {
    ...
    it('should throw error if query parameter not send', async () => {
        try {
            await request.get(helpers.API_URL + '/api/v1/search-partner-network', {
                json: true,
                headers: {
            ...
            const response = {
                code: 400,
                description: 'Please provide atleast one (partnerNetworkID, partnerNetworkNameand merchantID) parameter in query',
    ...
    it('should return document when all validation passes', async () => {
        ...
        expect(result[0]._id).to.equal(samplePartnerNetwork._id);
    });

    it('should return document when all validation passes with all parameter', async () => {
