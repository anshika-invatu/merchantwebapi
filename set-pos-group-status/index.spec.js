...
samplePointOfService.posGroupID = uuid.v4();
...
describe('Set POS Group Status', () => {
    ...
    it('should throw error when body not present.', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/${samplePointOfService.merchantID}/set-pos-group-status`, {
                json: true,
                headers: {
            ...
            const response = {
                code: 400,
                description: 'You\'ve requested to set a POS group status but the request body seems to be empty. Kindly specify status using request body in application/json format',
    ...
    it('should throw error on incorrect pointOfServiceID field', async () => {
        try {
            ...
            const response = {
                code: 400,
                description: 'The posGroupID field specified in the request body does not match the UUID v4 format.',
    ...
    it('should throw 404 error if the documentId is invalid', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/${samplePointOfService.merchantID}/set-pos-group-status`, {
                body: { statusCode: 'available', posGroupID: uuid.v4() },
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
            ...
            const response = {
                code: 404,
                description: 'The point-of-service of specified details doesn\'t exist.',
    ...
    it('should throw 404 error if statusCode is invalid', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/merchants/${samplePointOfService.merchantID}/set-pos-group-status`, {
                body: {
                    statusCode: 'test',
                    posGroupID: samplePointOfService.posGroupID
                },
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
            ...
            const response = {
                code: 403,
                description: 'You\'ve requested to set a POS group status with wrong statusCode.',
