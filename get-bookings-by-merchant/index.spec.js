...
describe('Get Bookings by merchant', () => {
    before(async () => {
    ...
    it('should return status code 400 when id not correct', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/bookings`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
    ...
    it('should return doc document when all validation passes', async () => {
        const result = await request.post(`${helpers.API_URL}/api/v1/merchants/${merchantID}/bookings`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(result[0]._id).to.eql(sampleBookings._id);
