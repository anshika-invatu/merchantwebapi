...
describe('deny-partner-member-request', () => {
    ...
    it('should return status code 400 when input query fields are missing', async () => {
        try {
            await request.patch(helpers.API_URL + `/api/v1/merchant/${uuid.v4()}/deny-partner-member-request`, {
                json: true,
                headers: {
    ...
    it('should throw error on incorrect _id field', async () => {
        try {
            await request.patch(helpers.API_URL + `/api/v1/merchant/${uuid.v4()}/deny-partner-member-request?partnerNetworkID=123&requestedMerchantID=${merchantID}`, {
                json: true,
                headers: {
                    'x-functions-key': process.env.X_FUNCTIONS_KEY,
                    'Authorization': authToken
