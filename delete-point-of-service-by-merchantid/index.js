...
//This endpoint delete pointOfService doc(bac-165)
    ...
    return request.get(`${process.env.USER_API_URL}/api/v1/users/${utils.decodeToken(req.headers.authorization)._id}`, {
        ...
        .then(() => {
            if (isDeviceAccessible) {
                return request.delete(`${process.env.DEVICE_API_URL}/api/v1/merchants/${req.params.id}/point-of-services?${context.req.originalUrl.split('?')[1]}`, {
                    json: true,
                    headers: {
        ...
        .then(pointOfService => {
            if (pointOfService) {
                ...
                logMessage.result = 'Delete point-of-service call completed successfully';
