...
describe('Get Merchant', () => {
    beforeEach(async () => {
        sampleUser.merchants[0].merchantID = sampleMerchant._id;
    ...
    it('should return merchant doc when all validation pass', async () => {
        ...
        const response = await request.get(`${helpers.API_URL}/api/v1/merchants/${sampleMerchant._id}`, {
            json: true,
            headers: {
    ...
    it('should throw 401 error if the documentId is not linked to user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
