...
describe('Get WorkLog', () => {
    ...
    it('should get doc when all validation pass', async () => {
        ...
        const response = await request.post(`${helpers.API_URL}/api/v1/merchants/${merchantID}/work-logs`, {
            json: true,
            headers: {
                'Authorization': authToken
            },
    ...
    it('should throw 401 error if the merchantID is not linked to user', async () => {
        try {
            await request.post(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/work-logs`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
