...
describe('Get OcpiLocation', () => {
    ...
    it('should get doc when all validation pass', async () => {
        ...
        const response = await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/ocpi-location/${sampleOcpiLocation._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(response._id).to.equal(sampleOcpiLocation._id);
    ...
    it('should throw 401 error if the merchantID is not linked to user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/ocpi-location/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
