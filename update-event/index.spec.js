...
describe('Update Event', () => {
    ...
    it('should throw error if id is not uuid.', async () => {
        try {
            await request.patch(`${helpers.API_URL}/api/v1/event/123-abc`, {
                json: true,
                headers: {
    ...
    it('should update doc when all validation pass', async () => {
        ...
        const result = await request.patch(`${helpers.API_URL}/api/v1/event/${sampleEvents._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            },
        ...
        expect(result.description).to.equal('Successfully updated the specified event.');
    });
    
    it('should not update doc when module not exist', async () => {

        try {
            await request.patch(`${helpers.API_URL}/api/v1/event/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
