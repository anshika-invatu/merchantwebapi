...
describe('Get WebhookSubscriber', () => {
    before(async () => {
    ...
    it('should get doc when all validation pass', async () => {
        ...
        const response = await request.get(`${helpers.API_URL}/api/v1/merchants/${merchantID}/webhook-subscriber/${sampleWebhookSubscribers._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
        ...
        expect(response._id).to.equal(sampleWebhookSubscribers._id);
    ...
    it('should throw 401 error if the documentId is not linked to user', async () => {
        try {
            await request.get(`${helpers.API_URL}/api/v1/merchants/${uuid.v4()}/webhook-subscriber/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                }
