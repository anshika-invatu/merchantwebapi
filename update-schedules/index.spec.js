...
describe('Update Schedules', () => {
    ...
    it('should update schedules doc when all validation pass', async () => {
        ...
        const result = await request.patch(`${helpers.API_URL}/api/v1/schedules/${sampleSchedules._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            },
        ...
        expect(result.description).to.be.equal('Successfully updated the specified schedules');

        await request.delete(`${helpers.API_URL}/api/v1/schedules/${sampleSchedules._id}`, {
            json: true,
            headers: {
                'Authorization': authToken
            }
    ...
    it('should not delete schedules when db don\'t have schedules with this id.', async () => {
        ...
        try {
            await request.patch(`${helpers.API_URL}/api/v1/schedules/${uuid.v4()}`, {
                json: true,
                headers: {
                    'Authorization': authToken
                },
            ...
            const response = {
                code: 404,
                description: 'The schedules specified details in the URL doesn\'t exist.',
                reasonPhrase: 'SchedulesNotFoundError'
            };
    ...
    it('should return error when id is not in uuid format', async () => {
        ...
        try {
            await request.patch(`${helpers.API_URL}/api/v1/schedules/123`, {
                json: true,
                headers: {
                    'Authorization': authToken
                },
